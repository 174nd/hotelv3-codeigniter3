...
class Admin_model extends CI_Model
{
  ...
  public function getReservationRooms($reservation_id, $extend_days = null)
  {
    ...
    $this->db->select("*, IF(payment_desciption='Deposit','disabled','') payment_status");
    return [
      'rooms'             => $rooms,
      'room_plan_id'      => $room_plan_id,
      'payment_histories' => $this->db->get_where("payment_histories", ['reservation_id' => $result['reservation_id']])->result_array(),
    ];
  }
  ...
  public function getData()
  {
    if ($this->input->post('set') == 'refresh_dashboard') {
      ...
      $this->db->select("segment_id AS `id`, CONCAT(segment_name, ' - ',segment_type) AS `text`");
      ...
      $this->db->select("*, COALESCE((SELECT GROUP_CONCAT(room_number SEPARATOR ', ') AS rooms FROM room_reservations JOIN `rooms` USING (`room_id`) WHERE room_reservations.reservation_id=reservations.reservation_id),'-') AS rooms");
      ...
      if ($receipt_type == 'invoice') {
        ...
        $payment_histories = [[
          ...
          'payment_desciption' => 'Remaining Payment',
          ...
          'payment_id'         => $payment_id,
          'payment_date'       => $checkout_date,
          'payment_number'     => $payment_number2,
          'payment_type'       => 'refund',
          'payment_desciption' => 'Deposit Refund',
          'total_payment'      => ($reservations['deposit'] * -1),
          'created_at'         => date('Y-m-d H:i:s'),
          'created_by'         => $this->session->userdata('user_id'),
        ]];
        $payment_number = $payment_number2;
        ...
        $payment_histories = [[
          ...
          'payment_date'       => $checkout_date,
          'payment_number'     => $payment_number,
          'payment_type'       => $remaining_payment > 0 ? 'remaining' : 'refund',
          'payment_desciption' => $remaining_payment > 0 ? 'Remaining Payment' : 'Deposit Refund',
          'total_payment'      => $remaining_payment,
          'created_at'         => date('Y-m-d H:i:s'),
          'created_by'         => $this->session->userdata('user_id'),
        ]];
      ...
      $query = $this->db->update('reservations', [
        ...
        'created_at'         => date('Y-m-d H:i:s'),
        'created_by'         => $this->session->userdata('user_id'),
      ]) && $this->db->insert_batch('payment_histories', $payment_histories) && $this->db->update_batch('rooms', $rooms, 'room_id') && $this->db->insert_batch('cleaning_histories', $cleaning_histories) && $this->db->insert_batch('room_reservation_histories', $room_reservation_histories);
