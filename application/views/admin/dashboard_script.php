  ...
  $(function() {
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data['status'] == 'done') {
              ...
              $('#reservation-fill #checkin_time').val(moment().format('HH:mm'));
    ...
    $('#reservation-fill #status').change(function() {
      if ($(this).val() == 'Check-In') {
        let val = $('#reservation-fill #segment_id').find("option:contains('Walk In - non-guaranted')").val();
    ...
    // Reservation Data
    var table_reservation_data = $('#table_reservation_data').attr('style', "width:100%; min-width:700px;").DataTable({
      'paging': false,
      'info': false,
      'searching': true,
      'ordering': true,
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $(data['reservations']).each(function(index, hasil) {
              ...
              table_reservation_data.row.add([hasil['reservation_number'], hasil['guest_name'], hasil['rooms'], hasil['in_house'], hasil_button]).draw().node();
            });
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $('#checkin-reservation #checkin_time').val(moment().format('HH:mm'));
    ...
    function setTableCheckinDetail(room_data) {
      ...
      $(room_data['rooms']).each(function(index, r1) {
        ...
        $(r1['room_data']).each(function(index, r2) {
          ...
          total += parseInt(r2['room_price']) * in_house;
          if (i == 0) {
            $('#checkin-detail table tbody').append('<tr><td colspan="3" rowspan="' + r2['row_number'] + '" class="align-middle">Room reserved type : ' + r2['room_type_name'] + ' No. ' + r2['room_number'] + sessions + '</td><td class="align-middle text-center">' + money_format(r2['room_price']) + '</td><td rowspan="' + r2['row_number'] + '" class="align-middle text-center">' + in_house + ' <?= $this->lang->line('text-night') ?></td><td rowspan="' + r2['row_number'] + '" class="align-middle text-center">' + money_format(parseInt(r2['room_price']) * in_house) + '</td><td rowspan="' + r1['row_number'] + '" class="align-middle text-center"><button type="button" class="btn btn-sm btn-info change_price" room_reservation_id="' + r1['room_reservation_id'] + '" in_house="' + in_house + '"><i class="fa fa-money-check-alt"></i></td></tr>');
          } else {
        ...
        $(r1['additional_costs']).each(function(index, r2) {
          ...
          $('#checkin-detail table tbody').append('<tr><td class="align-middle text-center" style="width: 40px;">' + cost_type + '</td><td colspan="4" class="align-middle">' + r2['additional_cost_description'] + '</td><td class="align-middle text-center">' + money_format(r2['additional_cost_price']) + '</td><td class="align-middle text-center"><button type="button" class="btn btn-sm btn-danger delete-additional_costs" additional_cost_id="' + r2['additional_cost_id'] + '" reservation_id="' + r1['reservation_id'] + '"><i class="fa fa-trash-alt"></i></td></tr>');
        });
      });
      $('#checkin-detail table tbody').append('<tr><th class="text-center align-middle" colspan="5"><?= $this->lang->line('text-total_price') ?></th><td class="text-center">' + money_format(total) + '</td><td></td></tr>');
      ...
      if (room_data['payment_histories'].length > 0) {
        $(room_data['payment_histories']).each(function(index, r1) {
          ...
          let button_delete = r1['total_payment'] == 'onstay' ? '<button type="button" class="btn btn-sm btn-danger delete-payment" payment_history_id="' + r1['payment_history_id'] + '" reservation_id="' + r1['reservation_id'] + '"><i class="fa fa-trash-alt"></i></button>' : '';
          $('#checkin-detail table tbody').append('<tr><td colspan="2" class="align-middle text-center" style="width: 100px;">' + setFullDate(r1['payment_date']) + '</td><td colspan="3" class="align-middle">' + r1['payment_desciption'] + '</td><td class="align-middle text-center">' + money_format(r1['total_payment']) + '</td><td class="align-middle text-center">' + button_delete + '</td></tr>');
        });

        let line = total > 0 ? '<?= $this->lang->line('text-remaining_payment') ?>' : '<?= $this->lang->line('text-remaining_return') ?>';
        $('#checkin-detail table tbody').append('<tr><th class="text-center align-middle" colspan="5">' + line + '</th><td class="text-center">' + money_format(Math.abs(total)) + '</td><td></td></tr>');
      }


      $('#checkout-reservation #remaining').val(total);
      return {
        'rooms': rooms,
        'total': total,
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $(data['reservations']).each(function(index, hasil) {
              ...
              table_checkin_data.row.add([hasil['reservation_number'], hasil['guest_name'], hasil['rooms'], hasil['in_house'], hasil_button]).draw().node();
            });
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $('#checkout-reservation #deposit').val(reservations['deposit']);
            $('#checkout-reservation #checkout_date').val(reservations['checkout_schedule']);
            $('#checkout-reservation #reservation_id').val(reservation_id);
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $('#checkout-reservation #checkout_time').val(moment().format('HH:mm'));
            $('#checkout-reservation #payment_id').prop('selectedIndex', 0);
            $('#checkout-reservation #receipt_type').prop('selectedIndex', 0).trigger('change');
    ...
    $('#checkout-reservation #receipt_type').change(function() {
      if ($(this).val() == 'bill') {
        let remaining = parseInt($('#checkout-reservation #remaining').val());
        $('#checkout-reservation #remaining_payment').val(remaining).trigger('input');
      } else {
        let deposit = parseInt($('#checkout-reservation #deposit').val());
        let remaining = parseInt($('#checkout-reservation #remaining').val());
        $('#checkout-reservation #remaining_payment').val(remaining + deposit).trigger('input');
      }
    });
      ...
      $.ajax({
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $('#add-payment #payment_date').attr('disabled', true).datepicker("setDate", moment().format('YYYY-MM-DD'));
          ...
          success: function(data) {
            if (data['status'] == 'done') {
              ...
              $(data['reservations']).each(function(index, hasil) {
                ...
                table_guest_data.row.add([hasil['reservation_number'], hasil['guest_name'], hasil['rooms'], hasil['in_house'], hasil_button]).draw().node();
              });
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            if (get_room['payment_histories'].length > 0) {
              let i = 0;
              let last = get_room['payment_histories'].length - 1;
              $(get_room['payment_histories']).each(function(index, r1) {
                let status = (
                  reservations['reservation_status'] == 'Finished' &&
                  ((reservations['receipt_type'] == 'invoice' && (i == 0 || i == last || i == (last - 1))) ||
                    (reservations['receipt_type'] == 'bill' && (i == last))
                  )
              ...
              if (reservations['reservation_status'] == 'Finished' && reservations['receipt_type'] == 'invoice') {
                $('#guest-detail table tbody').append('<tr><th class="text-center align-middle" colspan="5"><?= $this->lang->line('text-deposit_refund') ?></th><td class="text-center">' + money_format(Math.abs(reservations['deposit'])) + '</td><td></td></tr>');
              }
            }
