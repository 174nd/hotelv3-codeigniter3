      ...
      $.ajax({
        ...
        success: function(data) {
          if (data['status'] == 'done') {
            ...
            $('#guests-data .set-button .set-update').attr('href', '<?= base_url('guests/update/') ?>' + guest_id);
            $('#guests-data .set-button .set-delete').unbind().click(function() {
              $('#guests-delete #guest_id').val(guest_id);
              $('#guests-delete').modal('show');
            });
