...
class Guests extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      $this->load->view('templates/dashboard-admin', [
        'content'    => $this->load->view('guests/main', ['guest' => null], true),
        'add_css'    => $this->load->view('guests/add_css', null, true),
        'add_script' => $this->load->view('guests/add_script', null, true),
        'own_script' => $this->load->view('guests/main_script', null, true),
        'set'        => [
  ...
  public function update()
  {
    ...
    if ($guest_id != '') {
      ...
      if ($this->form_validation->run() == FALSE) {
        ...
        $this->load->view('templates/dashboard-admin', [
          'content'    => $this->load->view('guests/main', ['guest' => $guest], true),
          'add_css'    => $this->load->view('guests/add_css', null, true),
          'add_script' => $this->load->view('guests/add_script', null, true),
          'own_script' => $this->load->view('guests/main_script', null, true),
          'set'        => [
        ...
        $this->guest->updateGuests($guest_id, 'guests', 'guests/update/' . $guest_id);
      }
    } else {
      redirect('guests');
    }
  ...
  {
    if ($this->input->post('delete-guests')) {
      $this->guest->deleteGuests($this->input->post('guest_id'), 'guests');
    } else {
